...
namespace Codes
{
    public class CommonCode : CodeHelper
    {
        ...
        public static string GetCSharpCopyrightCode()
        {
            ...
            AppendFormatLine(code, 0, "// 创建标识: Copyright (C) {0} Socansoft.com 版权所有", DateTime.Today.Year);
        ...
        public static string GetSQLCopyrightCode(string function)
        {
            StringBuilder code = new StringBuilder();
            AppendFormatLine(code, 0, "-----------------------------------------------");
            AppendFormatLine(code, 0, "--Copyright (C) {0} Socansoft.com 版权所有", DateTime.Today.Year);
